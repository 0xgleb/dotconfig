...
;; Emacs 30+ ships a built-in editorconfig that lacks `editorconfig-exclude-regexps'.
;; Doom's editorconfig module expects it, so define it before the module loads.
(unless (boundp 'editorconfig-exclude-regexps)
  (defvar editorconfig-exclude-regexps ()
    "List of regexps for buffer filenames excluded from editorconfig."))
...
;; Recover from corrupted savehist files instead of showing an error on every startup
(defadvice! +savehist-recover-from-corruption-a (fn &rest args)
  :around #'savehist-mode
  (condition-case err
      (apply fn args)
    (error
     (message "savehist: corrupt history file, recreating (%s)" (error-message-string err))
...
`haskell-mode-buffer-apply-command'."
  ...
  (haskell-mode-buffer-apply-command "fourmolu"
...
;;; magit-butler: GitButler integration (disabled)
;; (use-package! magit-butler
;;   :commands (magit-butler-status magit-butler-dispatch)
;;   :init
;;   (map! :leader
;;         :desc "GitButler status" "g B" #'magit-butler-status)
;;   :config
;;   (after! magit
;;     (magit-butler-setup-magit-integration)))

;; ;; Open magit-butler as the initial buffer when Emacs starts
;; (setq initial-buffer-choice
;;       (lambda ()
;;         (magit-butler-status)
;;         (get-buffer "*magit-butler*")))
