...
test("fleet diagnostics expose current behavior-bearing component versions", () => {
  assert.match(read("../pi/extensions/shared/runtime-version.ts"), /MANAGED_CONFIG_GENERATION = "2026\.07\.23\.126"/);
  assert.match(read("../pi/extensions/activity-status/index.ts"), /activity-status", "2026\.07\.23\.2"/);
  assert.match(read("../pi/extensions/agent-registry/index.ts"), /agent-registry", "2026\.07\.23\.17"/);
  ...
  assert.match(read("../pi/extensions/classified-workflows/index.ts"), /classified-workflows", "2026\.07\.23\.92"/);
  assert.match(read("../pi/extensions/compact-read/index.ts"), /compact-read", "2026\.07\.31\.2"/);
  ...
  assert.match(read("../pi/extensions/pi-vim/index.ts"), /pi-vim", "2026\.07\.23\.8"/);
  assert.match(read("../pi/extensions/release-cadence/index.ts"), /release-cadence", "2026\.07\.23\.2"/);
  ...
  assert.match(read("../pi/extensions/todo/index.ts"), /todo", "2026\.07\.23\.13"/);
});
