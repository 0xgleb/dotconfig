...
test("fleet diagnostics expose current behavior-bearing component versions", () => {
  assert.match(
    ...
    /MANAGED_CONFIG_GENERATION = "2026\.07\.23\.161"/,
  )
  assert.match(
  ...
  assert.match(
    ...
    /browser-control", "2026\.09\.14\.1"/,
  )
  assert.match(read("../pi/extensions/btw/index.ts"), /btw", "2026\.07\.23\.1"/)
  assert.ok(
    /classified-workflows", "2026\.09\.19\.10"/.test(
      read("../pi/extensions/classified-workflows/index.ts"),
    ),
    "classified-workflows marker must match 2026.09.19.10",
  )
  assert.match(
    read("../pi/extensions/compact-read/index.ts"),
    /compact-read", "2026\.08\.09\.1"/,
  )
  assert.match(read("../pi/extensions/lsp/index.ts"), /lsp", "2026\.09\.04\.3"/)
  assert.match(
