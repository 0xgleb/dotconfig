...
test("fleet diagnostics expose current behavior-bearing component versions", () => {
  assert.match(read("../pi/extensions/shared/runtime-version.ts"), /MANAGED_CONFIG_GENERATION = "2026\.07\.23\.106"/);
  assert.match(read("../pi/extensions/activity-status/index.ts"), /activity-status", "2026\.07\.23\.2"/);
  assert.match(read("../pi/extensions/agent-registry/index.ts"), /agent-registry", "2026\.07\.23\.15"/);
  ...
  assert.match(read("../pi/extensions/classified-workflows/index.ts"), /classified-workflows", "2026\.07\.23\.73"/);
