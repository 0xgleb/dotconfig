...
test("Home Manager runs the owner-only Telegram bridge from the ragenix token path", () => {
  ...
  assert.match(home, /PIECE_OF_PI_TELEGRAM_TOKEN_FILE = config\.age\.secrets\.metagenda-telegram-token\.path/)
...
test("Telegram telemetry is structured and excludes protected values", () => {
  ...
  assert.match(threatModel, /token, message text, username, numeric user ID, session ID, and response text are absent from telemetry/)
})
