...
test("EOD uses stakeholder headers and reference-adjacent counts", () => {
  ...
  assert.match(eod, /Never emit generic headings.*`What Was Done`.*`Review hardening`/is);
...
test("EOD preserves concurrent manual edits through live anchor-only patches", () => {
  assert.match(eod, /re-read the live target immediately before every mutation/i);
  assert.match(eod, /use `Edit` only on the exact placeholder or\s+user-requested anchor/is);
...
test("EOD reconciles exact Graphite batches instead of broad merged searches", () => {
  ...
  assert.match(eod, /Never replace this bounded\s+scope with broad `is:merged`, `mergedAt`/is);
