...
import { availableMemoryBytes, parseMemoryPressureCapacity } from "./memory-capacity.ts";
...
test("non-macOS available memory uses the operating system free-memory probe", () => {
  assert.equal(
    availableMemoryBytes({ platform: "linux", freeMemoryBytes: () => 5_583_457_280 }),
    5_583_457_280n,
  );
});
...
test("macOS available-memory failures fail closed", () => {
  assert.throws(
    () => availableMemoryBytes({ platform: "darwin", memoryPressureProbe: () => ({ status: 1, stdout: "" }) }),
    /probe failed/,
  );
  assert.throws(
    () => availableMemoryBytes({
