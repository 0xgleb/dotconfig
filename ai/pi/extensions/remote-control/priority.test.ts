...
test("only authenticated Telegram ingress records a direct owner intervention", () => {
  ...
  assert.match(
    source,
    /if \(remoteSourceCarriesOwnerAuthority\(source\) && prompt\?\.type === "text"\)[\s\S]*?pi\.events\.emit\(HUMAN_TURN_EVENT, prompt\.text\)[\s\S]*?pi\.sendUserMessage\(content,/,
  )
})
...
test("owner-relay frames reach Telegram before the bridge message completes", () => {
  assert.match(
    source,
    /const relay = parseOwnerRelay\(message\.text\);[\s\S]*?deliverOwnerRelay\(relay, message\.requesterId\)[\s\S]*?store\.complete\(\{/,
  )
  assert.match(
    source,
