...
export type RemoteFailure = "aborted" | "bridge_disabled" | "expired" | "model_error" | "session_ended";
...
export const boundedBridgeText = (label: string, text: string, maximum: number): string => {
...
export const boundedIdentifier = (label: string, value: string, maximum = 128): string => {
  const bounded = boundedBridgeText(label, value, maximum);
  if (!/^[A-Za-z0-9._:-]+$/.test(bounded)) {
    throw new RemoteBridgeError({ code: "invalid_input", message: `${label} has invalid characters` });
  }
  return bounded;
};
...
export const boundedTimestamp = (label: string, value: number): number => {
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new RemoteBridgeError({ code: "invalid_input", message: `${label} must be a timestamp` });
  }
  return value;
};
...
export const boundedTtl = (value: number): number => {
  if (!Number.isSafeInteger(value) || value < 1_000 || value > 60 * 60_000) {
    throw new RemoteBridgeError({ code: "invalid_input", message: "ttl must be between 1s and 1h" });
  }
  return value;
};
...
export const finalAssistantText = (messages: readonly unknown[]): string | undefined => {
