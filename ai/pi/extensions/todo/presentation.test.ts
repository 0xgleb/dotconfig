...
test("todo summary counts pending and completed tasks", () => {
  assert.deepEqual(todoSummary(state), { total: 4, completed: 1, pending: 3 });
});
...
test("kanban columns separate current, queued, and completed work", () => {
  ...
  assert.deepEqual(columns.next.map(({ id }) => id), [3, 4]);
...
test("task widget lines show compact top active tasks", () => {
  assert.deepEqual(taskWidgetLines(state, 2), [
    "Tasks: 1/4 done · 3 active · /kanban",
    "○ #2 Fix classifier",
    "○ #3 Add task overlay",
