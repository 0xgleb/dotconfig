...
test("handshakes are accepted only for the provisioned extension origin", () => {
  ...
  const rejectedOrigin = decideHandshake(
    {
      ...validHandshake,
      headers: { ...validHandshake.headers, origin: "https://example.com" },
    },
    extensionOrigin,
  )
  assert.equal(rejectedOrigin.outcome, "rejected")
  assert.equal(
    decideHandshake({ ...validHandshake, method: "POST" }, extensionOrigin)
      .outcome,
    "rejected",
  )
  assert.equal(
    decideHandshake(
      {
        ...validHandshake,
        headers: { ...validHandshake.headers, "sec-websocket-version": "8" },
      },
      extensionOrigin,
    ).outcome,
    "rejected",
  ...
  assert.equal(
    decideHandshake(
      {
        ...validHandshake,
        headers: { ...validHandshake.headers, "sec-websocket-key": undefined },
      },
      extensionOrigin,
    ).outcome,
    "rejected",
  ...
  assert.equal(
    decideHandshake(
      {
        ...validHandshake,
        headers: { ...validHandshake.headers, "sec-websocket-key": "tooshort" },
      },
      extensionOrigin,
    ).outcome,
    "rejected",
  ...
  assert.equal(
    decideHandshake(
      {
        ...validHandshake,
        headers: { ...validHandshake.headers, upgrade: "h2c" },
      },
      extensionOrigin,
    ).outcome,
    "rejected",
...
test("protocol violations fail the whole connection", () => {
  ...
  assert.equal(
    decodeClientFrames(unmasked, MAX_CLIENT_FRAME_BYTES).outcome,
    "violation",
  )
  const fragmented = maskFrame([0x01, 0x81], Uint8Array.from([0x68]))
  assert.equal(
    decodeClientFrames(fragmented, MAX_CLIENT_FRAME_BYTES).outcome,
    "violation",
  )
  const binary = maskFrame([0x82, 0x81], Uint8Array.from([0x68]))
  assert.equal(
    decodeClientFrames(binary, MAX_CLIENT_FRAME_BYTES).outcome,
    "violation",
  )
  const reserved = maskFrame([0xc1, 0x81], Uint8Array.from([0x68]))
  assert.equal(
    decodeClientFrames(reserved, MAX_CLIENT_FRAME_BYTES).outcome,
    "violation",
  )
  ...
  const extendedControl = maskFrame(
    [0x89, 0xfe, 0x00, 0x7e],
    new Uint8Array(126),
  )
  assert.equal(
    decodeClientFrames(extendedControl, MAX_CLIENT_FRAME_BYTES).outcome,
    "violation",
  )
})

test("server frames are unmasked and bounded", () => {
  const text = run(encodeTextFrame("ok"))
  assert.deepEqual(Array.from(text), [0x81, 0x02, 0x6f, 0x6b])
  const longText = run(encodeTextFrame("x".repeat(200)))
  assert.deepEqual(Array.from(longText.slice(0, 4)), [0x81, 0x7e, 0x00, 0xc8])
  assert.equal(longText.length, 4 + 200)
  const pong = run(encodePongFrame(Uint8Array.from([0x01])))
...
test("transport tokens are safe identifiers minted from sufficient entropy", () => {
  ...
  const token = run(mintTransportToken(entropy))
  assert.equal(token.length, TRANSPORT_TOKEN_ENTROPY_BYTES * 2)
  assert.match(token, /^[a-f0-9]+$/)
  assert.throws(() => run(mintTransportToken(new Uint8Array(8))))
})
...
test("hello admission requires the provisioned token and operator profile", () => {
  const provisioned = {
    token: run(
      mintTransportToken(
        new Uint8Array(TRANSPORT_TOKEN_ENTROPY_BYTES).fill(0x77),
      ),
    ),
  ...
  assert.equal(
    decideHello(
      { ...hello, token: provisioned.token.replace("7", "8") },
      provisioned,
    ).outcome,
    "rejected",
  )
  assert.equal(
    decideHello({ ...hello, profilePath: "/tmp/other-profile" }, provisioned)
      .outcome,
    "rejected",
  )
  assert.equal(
    decideHello({ malformed: true }, provisioned).outcome,
    "rejected",
  )
  assert.equal(
    decideHello(
