...
// Source identity and cumulative token usage are additive so sessions still
// running the prior adapter can coexist during rolling Pi reloads.
...
const rowFrom: (value: unknown) => Row = value => {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw registryError(
      "corrupt_state",
      "registry query returned a malformed row",
    )
  }
  return value
}
...
const rowsFrom: (value: unknown) => readonly Row[] = value => {
  if (!Array.isArray(value))
    throw registryError(
      "corrupt_state",
      "registry query returned malformed rows",
    )
...
const boundedText: (label: string, text: string, maximum: number) => string = (
  label,
  text,
  maximum,
) => {
...
const canonicalProject: (project: string) => string = project => {
  ...
  if (
    ...
    throw registryError(
      "invalid_input",
      "project must be a bounded absolute path",
    )
  }
...
const validateAgent: (agent: AgentIdentity) => AgentIdentity = agent => {
  ...
  if (!Number.isSafeInteger(agent.pid) || agent.pid < 1)
    throw registryError("invalid_input", "pid must be positive")
...
const validateTokenUsage = (usage: AgentTokenUsage): AgentTokenUsage => {
  ...
  if (
    ...
    throw registryError(
      "invalid_input",
      "agent token usage must contain non-negative safe integers",
    )
  }
  return usage
}
...
const validateActivities = (
  ...
  return activities.map(activity => {
    ...
    if (
      ...
      throw registryError("invalid_input", "agent activity status is malformed")
    }
    return {
...
): RegistryRequestPriority => {
  if (value === "normal" || value === "urgent") return value
  throw registryError(failure, "registry request priority is malformed")
}

const validateTime: (label: string, value: number) => number = (
  label,
  value,
) => {
  if (!Number.isSafeInteger(value) || value < 0)
    throw registryError("invalid_input", `${label} must be a timestamp`)
  return value
}
...
const expiresAt: (now: number, ttlMs: number) => number = (now, ttlMs) => {
  ...
  if (!Number.isSafeInteger(expiration))
    throw registryError(
      "invalid_input",
      "lease expiration exceeds safe time range",
    )
  return expiration
}
...
const stringField: (
  ...
  key: string,
  optional?: boolean,
) => string | undefined = (row, key, optional = false) => {
  const value = row[key]
...
const optionalNumberField: (row: Row, key: string) => number | undefined = (
  row,
  key,
) => {
  const value = row[key]
...
const leaseFromRow: (row: Row) => Lease = row => {
  ...
  const owner: AgentIdentity = {
    ...
    ...(stringField(row, "owner_model", true)
      ? { model: stringField(row, "owner_model", true) }
      : {}),
    ...(runtimeVersions ? { runtimeVersions } : {}),
  }
  const base = {
...
const activitiesFromRow = (row: Row): readonly AgentActivity[] => {
  const encoded = stringField(row, "activities", true)
  if (encoded === undefined) return []
  try {
    const parsed: unknown = JSON.parse(encoded)
    if (!Array.isArray(parsed)) throw new Error("activities must be an array")
    return validateActivities(parsed as readonly AgentActivity[])
  } catch {
    throw registryError(
      "corrupt_state",
...
const registeredAgentFromRow = (row: Row): RegisteredAgent => {
  const runtimeVersions = runtimeVersionsFromRow(row)
  const activities = activitiesFromRow(row)
  return {
    identity: {
      id: stringField(row, "agent_id") ?? "",
      pid: numberField(row, "pid"),
      ...(stringField(row, "model", true)
        ? { model: stringField(row, "model", true) }
        : {}),
      ...(runtimeVersions ? { runtimeVersions } : {}),
    },
    ...
    usage: {
      input: numberField(row, "usage_input"),
      output: numberField(row, "usage_output"),
      cacheRead: numberField(row, "usage_cache_read"),
      cacheWrite: numberField(row, "usage_cache_write"),
      totalTokens: numberField(row, "usage_total"),
    },
...
const requestFromRow: (row: Row) => RegistryRequest = row => {
  const status = stringField(row, "status")
  const requesterAcknowledgedAt = optionalNumberField(
    row,
    "requester_acknowledged_at",
  )
  const recipientReceivedAt = optionalNumberField(row, "recipient_received_at")
  const recipientAgentId = stringField(row, "recipient_agent_id", true)
  const recipientLeaseId = stringField(row, "recipient_lease_id", true)
  const receiptFields = [
    recipientReceivedAt,
    recipientAgentId,
    recipientLeaseId,
  ...
  const base = {
    ...
    ...(stringField(row, "requester_label", true)
      ? { requesterLabel: stringField(row, "requester_label", true) }
      : {}),
    ...(stringField(row, "requester_cwd", true)
      ? { requesterCwd: stringField(row, "requester_cwd", true) }
      : {}),
    text: stringField(row, "text") ?? "",
    priority: requestPriority(stringField(row, "priority")),
...
const tableColumns = (
  ...
  new Set(
    database
      ...
      .all()
      .map(row => stringField(rowFrom(row), "name") ?? ""),
  )
...
const initialize: (database: DatabaseSync, databasePath: string) => void = (
  database,
  databasePath,
) => {
  database.exec(`PRAGMA busy_timeout = ${BUSY_TIMEOUT_MS};`)
  const journalMode = stringField(
    rowFrom(database.prepare("PRAGMA journal_mode").get()),
    "journal_mode",
  )
  ...
  if (
    ...
    currentAdditiveSchemaInstalled(database)
  )
    return
  if (
    observedVersion !== 0 &&
  ...
  try {
    ...
    if (currentVersion === 0) {
      ...
      const addAgentColumn = (name: string, declaration: string) => {
        if (agentColumns.has(name)) return
        database.exec(`ALTER TABLE agents ADD COLUMN ${declaration};`)
        agentColumns.add(name)
      }
      addAgentColumn("usage_input", "usage_input INTEGER NOT NULL DEFAULT 0")
      addAgentColumn("usage_output", "usage_output INTEGER NOT NULL DEFAULT 0")
      addAgentColumn(
        "usage_cache_read",
        "usage_cache_read INTEGER NOT NULL DEFAULT 0",
      )
      ...
      addAgentColumn("usage_total", "usage_total INTEGER NOT NULL DEFAULT 0")
      addAgentColumn("activities", "activities TEXT")
      ...
      addColumn("recipient_received_at", "recipient_received_at INTEGER")
      addColumn("recipient_agent_id", "recipient_agent_id TEXT")
      addColumn("recipient_lease_id", "recipient_lease_id TEXT")
...
const requestRow: (database: DatabaseSync, requestId: string) => Row = (
  database,
  requestId,
) => {
  const row = optionalRowFrom(
...
export const makeSqliteRegistryStore: (
  ...
  return {
      ...
      effect("Could not claim role", () =>
        withDatabase(databasePath, database =>
          transaction(database, (): ClaimLeaseResult => {
            ...
            const policyDigest = boundedText(
              "policy digest",
              input.policyDigest,
              256,
            )
            ...
            if (existingRow) {
              ...
              if (existing.expiresAt > now)
                return { outcome: "already_owned", lease: existing }
              database
                .prepare("DELETE FROM leases WHERE project = ? AND role = ?")
                .run(project, role)
            }
            if (count(database, "leases") >= MAX_LEASES)
              throw registryError("capacity", "lease capacity reached")
            const lease: Lease = {
              id: randomUUID(),
              project,
              role,
      ...
      effect("Could not heartbeat lease", () =>
        withDatabase(databasePath, database =>
          transaction(database, (): Lease => {
            ...
            const runtimeVersions = validateRuntimeVersions(
      ...
      effect("Could not pause lease", () =>
        withDatabase(databasePath, database =>
          transaction(database, (): Lease => {
            ...
            database
              .prepare("UPDATE leases SET status = 'paused' WHERE lease_id = ?")
              .run(lease.id)
      ...
      effect("Could not queue request", () =>
        withDatabase(databasePath, database =>
          transaction(database, (): RegistryRequest => {
            if (count(database, "requests") >= MAX_REQUESTS)
              throw registryError("capacity", "request capacity reached")
            const now = validateTime("now", input.now)
            const request: RegistryRequest = {
              ...
              ...(input.requesterLabel
                ...
                : {}),
              ...(input.requesterCwd
                ? { requesterCwd: canonicalProject(input.requesterCwd) }
                : {}),
    ...
    receiveRequest: (input: ReceiveRequestInput) =>
      effect("Could not receive request", () =>
        withDatabase(databasePath, database =>
          transaction(database, (): RegistryRequest => {
            const now = validateTime("now", input.now)
            ...
            const request = requestFromRow(
              requestRow(database, input.requestId),
            )
            if (
            ...
            const result = database
              ...
              .run(now, lease.owner.id, lease.id, request.id)
            if (Number(result.changes) !== 1) {
              throw registryError(
                "invalid_transition",
    ...
    clearExceptProject: (input: ClearExceptProjectInput) =>
      effect("Could not clear registry", () =>
        withDatabase(databasePath, database =>
          transaction(database, (): ClearedRegistryCounts => {
            validateTime("now", input.now)
            const preservedProject = canonicalProject(input.preservedProject)
            const requestIds = rowsFrom(
              database
              ...
              .filter(
                row =>
                  !isWithinProject(
                    canonicalProject(stringField(row, "project") ?? ""),
                    preservedProject,
                  ),
              )
              .map(row => stringField(row, "request_id") ?? "")
            const leaseIds = rowsFrom(
              database.prepare("SELECT lease_id, project FROM leases").all(),
            )
              .filter(
                row =>
                  !isWithinProject(
              ...
              .map(row => stringField(row, "lease_id") ?? "")
            const agentIds = rowsFrom(
              database.prepare("SELECT agent_id, cwd FROM agents").all(),
            )
              .filter(
                row =>
                  !isWithinProject(
                    canonicalProject(stringField(row, "cwd") ?? ""),
                    preservedProject,
                  ),
              )
              .map(row => stringField(row, "agent_id") ?? "")
      ...
      effect("Could not fail request", () =>
        withDatabase(databasePath, database =>
          transaction(database, (): RegistryRequest => {
            ...
            const diagnostic = persistedText(
              "diagnostic",
              input.diagnostic,
              MAX_SUMMARY_TEXT,
            )
