...
export const isExpensiveCommand: (command: string) => boolean = command =>
  ...
  /(?:^|[\r\n;&|()]|\bsudo\s+)\s*\^?(?:direnv\s+exec\s+[^;&|`\s]+\s+)?(?:darwin-rebuild\s+(?:build|switch)|nixos-rebuild\s+(?:build|switch)|nix\s+(?:build|develop|run|flake\s+check)|cargo\s+(?:build|check|test|clippy|nextest)|(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?(?:build|test)|forge\s+(?:build|test)|docker\s+build|terraform\s+(?:plan|apply)|make(?:\s|$))/i.test(
    command,
  )
