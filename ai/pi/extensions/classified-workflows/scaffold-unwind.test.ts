...
const forward = {
  ...
  edits: [{ oldText: "existing test\n", newText: "existing test\nnew failing ADR43 test\n" }],
};
const inverse = {
  path: forward.path,
  edits: [{ oldText: forward.edits[0]?.newText, newText: forward.edits[0]?.oldText }],
};
...
test("exact inverse of agent-added scaffolding is allowed after owner reprioritization", () => {
  assert.equal(exactScaffoldUnwindDisprovesBlock({
    reason: "Removing this failing test would weaken the required TTDD workflow.",
    edit: inverse,
    branch,
    cwd: "/repo",
  }), true);
...
test("newly written uncommitted scaffolding can be emptied after its lane is deferred", () => {
  ...
  assert.equal(exactScaffoldUnwindDisprovesBlock({
    ...
    edit: { path: "tests/new-adr43.e2e.ts", edits: [{ oldText: content, newText: "" }] },
    branch: [
...
test("unwind remains blocked without an exact successful inverse and later human reprioritization", () => {
  const cases = [
    ...
    { branch: [call("edit-1", "edit", forward), result("edit-1", true), user("Defer ADR43")], edit: inverse },
    {
      branch,
      edit: { ...inverse, edits: [{ oldText: inverse.edits[0]?.oldText, newText: "different content" }] },
    },
  ];
...
test("independent committed or user-owned test concerns never get overridden", () => {
  for (const reason of [
    ...
    assert.equal(exactScaffoldUnwindDisprovesBlock({ reason, edit: inverse, branch, cwd: "/repo" }), false);
  }
});
