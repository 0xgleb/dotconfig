...
import { wordForward, WORDForward, charLeft, charRight, goToLastLine } from "../motions.ts";
...
test("visual j and k move by logical lines without delegating to wrapped-row arrows", () => {
  const e = editor(`${"a".repeat(500)}\nshort\nthird`, 0, 3);
  e.visual.superHandleInput = () => assert.fail("visual vertical motions must not use base-editor arrows");
  e.key("v");
