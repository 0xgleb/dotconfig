...
function replayLastChange(ctx: NormalModeContext, countOverride: number): void {
  ...
  try {
    ...
    const keys =
      countOverride > 0
        ? [...String(countOverride), ...change.keys.slice(firstCommand < 0 ? 0 : firstCommand)]
        : change.keys;
...
function paste(
  ...
  if (linewise) {
    const pasteLines = Array.from({ length: count }, () => text).join("\n").split("\n");
    if (before) {
    ...
    if (before) {
      ...
      const newLine = line.substring(0, cursor.col) + repeatedText + line.substring(cursor.col);
      lines[cursor.line] = newLine;
      ctx.setText(lines.join("\n"));
      ...
      if (pasteLines.length === 1) {
        const newLine = line.substring(0, insertCol) + repeatedText + line.substring(insertCol);
        lines[cursor.line] = newLine;
        ctx.setText(lines.join("\n"));
...
function moveLogicalVertical(ctx: NormalModeContext, direction: -1 | 1, count: number): void {
  const lines = ctx.getText().split("\n");
  const target = logicalVerticalPosition(lines, ctx.getCursor(), direction, count);
