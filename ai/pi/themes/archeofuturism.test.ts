...
const theme = JSON.parse(readFileSync(new URL("./archeofuturism.json", import.meta.url), "utf8")) as {
  name?: unknown;
...
test("archeofuturism defines the complete Pi theme surface", () => {
  ...
  for (const token of requiredColors) assert.ok(theme.colors?.[token] !== undefined, `missing ${token}`);
});

test("syntax classes are distinct and readable on the dark base", () => {
  const resolved = syntaxColors.map((token) => resolveColor(theme.colors?.[token]));
...
const contrastRatio: (left: string, right: string) => number = (left, right) => {
