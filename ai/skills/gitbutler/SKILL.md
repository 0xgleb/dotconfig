...
Use GitButler CLI (`but`) as the default version-control interface only in the repository's main worktree.

## Applicability Gate

Before running any `but` command, compare the current top-level from `git rev-parse --show-toplevel` with the first `worktree` path from `git worktree list --porcelain`. GitButler is valid only when those paths identify the same main worktree.

In a linked, isolated, scratch, or otherwise non-main worktree:
...
In the main worktree, GitButler applies. Before any history edit or conflict resolution, compare `but --version` with the frontmatter `version`. If they differ, read the installed command help before acting. Do not translate syntax from a stale version report.
...
1. In the verified main worktree, use `but` for all write operations. Never run `git add`, `git commit`, `git push`, `git checkout`, `git merge`, `git rebase`, `git stash`, or `git cherry-pick`; translate those writes to `but`. Exceptions in the main worktree are limited to `git add -- <path>` for a conflicted uncommitted file as documented below. In every non-main worktree, this rule reverses: never run GitButler, and use plain Git for all authorized reads and writes without waiting for `but` to reject the operation.
...
- Read-only Git inspection (`git log`, `git blame`, `git show --stat`) is allowed everywhere. In non-main worktrees, plain Git is also the required write interface.
