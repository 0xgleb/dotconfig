...
name: audit
user-invocable: true
allowed-tools: Bash(git:*), Bash(gh:*), Bash(gt:*), Bash(but:*), Bash(direnv:*), Bash(linear:*), Bash(cursor-agent:*), Bash(agy:*), Bash(command:*), Bash(cargo:*), Bash(nix:*), Bash(mkdir:*), Bash(cat:*), Bash(mktemp:*), Bash(rm:*), Bash(test:*), Bash(grep:*), Bash(wc:*), Bash(date:*), Bash(basename:*), Bash(find:*), Read, Write, Edit, Agent, Workflow, AskUserQuestion, Skill
...
the fix branches). For GitButler, confirm workspace mode (`but status` ok) as
`/review-sweep` does; if `but` is not on PATH, invoke it via `direnv exec
"$repo_root" but`.
...
7. The review runs as a single `Workflow` invocation per pass (review-core) —
   ...
   cursor-agent `--mode plan`; agy `--sandbox` and never
