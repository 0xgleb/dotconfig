...
description: Review trading-system and strategy code for quantitative-trading defects — flags look-ahead bias and data leakage in backtests or signals, mishandled order lifecycle and partial fills, ignored fees and slippage, stale or unsynchronized market data, tick and lot size violations, side and sign convention errors, and wrong PnL math. Auto-runs when a review or audit touches orders, fills, positions, market data, strategies, or backtests.
...
- **Signals & backtests:** feature computation, rolling windows,
  ...
  (or their absence), strategy entry/exit conditions.
...
1. **Stay in the trading-logic lane.** Data-to-decision-to-execution-to-
   position. No money representation, no risk limits, no general
   correctness.
