...
import { answerText, buildBoundedTranscript, sideQuestionPrompt, type TranscriptEntryLike } from "./core.ts";
...
class BtwAnswerPanel {
  ...
  render(width: number): string[] {
    ...
    const row = (text = "") => `${border("│")}${padded(` ${text}`, innerWidth)}${border("│")}`;
    const questionLines = wrapTextWithAnsi(this.question, contentWidth).slice(0, 4);
    ...
    const scrollLabel = answerLines.length > visibleLines
    ...
    return [
      ...
      row(),
      row(this.theme.fg("dim", "↑↓/Pg scroll · Enter/Esc close · not added to main context")),
...
const askSideQuestion = async (
  ...
  if (!auth.ok || !auth.apiKey) {
    return { status: "error", error: auth.ok ? `No API key for ${ctx.model.provider}.` : auth.error };
  }
  const transcript = buildBoundedTranscript(ctx.sessionManager.buildContextEntries() as TranscriptEntryLike[]);
  ...
  const result = await ctx.ui.custom<BtwResult>((tui, theme, _keybindings, done) => {
    ...
    void completeSimple(
      ...
      finish(answer
        ? { status: "ok", answer }
        : { status: "error", error: "Side question completed without a text answer." });
    }).catch((error: unknown) => {
    ...
    overlay: true,
    overlayOptions: { anchor: "center", width: "64%", minWidth: 52, maxHeight: "72%", margin: 1 },
  });
...
export default function btwExtension(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("btw", {
    description: "Ask an ephemeral side question without adding it to the main session context",
    handler: async (args, ctx) => {
      ...
      if (result.status === "error") {
        ctx.ui.notify(`/btw failed: ${result.error ?? "unknown error"}`, "error");
        return;
      }
      ...
      await ctx.ui.custom<void>((tui, theme, _keybindings, done) =>
        ...
        overlay: true,
        overlayOptions: { anchor: "center", width: "72%", minWidth: 52, maxHeight: "82%", margin: 1 },
      });
    },
  });
}
