...
const makeStore = (database: DatabaseSync): SqliteJobStore => {
  ...
  const claimDue: SqliteJobStore["claimDue"] = (
    ...
    inTransaction(
      Effect.gen(function* () {
        if (
          (kinds !== undefined && !isRegisteredKindFilter(kinds)) ||
          kinds?.includes("harness.research")
        ) {
          return yield* Effect.fail(
            storeError(
              "invalid_input",
              "generic claims require distinct non-research job kinds",
            ),
          )
        }
        const kindFilter =
          kinds === undefined
            ? " AND kind <> 'harness.research'"
      ...
      Effect.gen(function* () {
        ...
        return yield* Effect.forEach(rows, row =>
          Effect.flatMap(documentFromRow(row), job =>
            Effect.flatMap(recoverExpiredJob(job, now, retryDelayMs), persist),
          ),
        )
      }),
    )
    ...
    Effect.flatMap(
      sql(
        () =>
          database
            .prepare(
              "SELECT document FROM jobs ORDER BY updated_at DESC, job_id",
            )
            .all(),
        "failed to list jobs",
      ),
...
export const isRegisteredKindFilter = (
  ...
  kinds.every(kind => REGISTERED_JOB_KINDS.includes(kind as RegisteredJobKind))
