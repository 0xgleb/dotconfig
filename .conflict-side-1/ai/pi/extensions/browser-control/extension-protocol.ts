...
export class BrowserProtocolError extends Data.TaggedError(
...
export const decodeExtensionFrame: (
  ...
  if (value.kind === "hello") {
    if (
      ...
      return invalid(
        "hello frame requires a bounded token and absolute profile path",
      )
    }
...
export const decodeHostFrame: (
  ...
  if (
    ...
    return invalid(
      "host frame must carry the supported version and a safe request id",
    )
  }
  const requestId = value.requestId
  if (value.kind === "overlay") {
    if (
      ...
      return invalid(
        "overlay frame requires a typed phase and bounded printable label",
      )
    }
    return Effect.succeed({
      version: EXTENSION_PROTOCOL_VERSION,
...
export const resultMatchesAction: (
  frame: ResultFrame,
  action: BrowserAction,
) => boolean = (frame, action) =>
...
const decodeResultPayload = (
  ...
  if (!isRecord(value))
    return invalid("completed result requires a typed payload")
  if (value.action === "status") {
    ...
    for (const candidate of value.tabs) {
      ...
      if (tab === undefined) {
        return invalid(
          "status payload tabs must be bounded printable summaries",
        )
      }
      tabs.push(tab)
    }
