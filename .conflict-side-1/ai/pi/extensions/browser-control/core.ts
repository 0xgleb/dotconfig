...
export const launchServicesRequest: (
  ...
  if (!operatorProfilePath.startsWith("/"))
    throw new Error("Operator profile path must be absolute.")
  if (
...
export const collectBoundedResponseBytes = (
  ...
  if (!Number.isSafeInteger(maxBytes) || maxBytes < 1)
    throw new Error("Response byte limit must be positive.")
  const totalBytes = chunks.reduce(
    (total, chunk) => total + chunk.byteLength,
    0,
  )
  if (totalBytes > maxBytes)
    throw new Error(`Loopback response exceeded the ${maxBytes}-byte limit.`)
...
export const parseDebugTargets: (
  value: unknown,
  debugPort: number,
) => readonly DebugTarget[] = (value, debugPort) => {
  if (!Array.isArray(value))
    throw new Error("Brave debug target response must be an array.")
...
export const selectActiveTarget: (
  ...
  if (!activeTargetId)
    throw new Error("No explicitly opened local page is available.")
  const target = targets.find(candidate => candidate.id === activeTargetId)
  if (!target)
    throw new Error("The explicitly opened local page is no longer available.")
  return target
}
...
export const parseCdpResponse: (
  ...
  if (!isRecord(value))
    throw new Error("Brave returned a malformed CDP response.")
  ...
  if ("error" in value) {
    if (!isRecord(value.error))
      throw new Error("Brave returned a malformed CDP error.")
    const dataMessage =
...
const parseDebugTarget: (
  value: unknown,
  debugPort: number,
) => DebugTarget | undefined = (value, debugPort) => {
  if (!isRecord(value))
    throw new Error("Brave returned a malformed debug target.")
