...
const decoded = <A>(effect: Effect.Effect<A, unknown>): A =>
...
test("act frames correlate their parameters with the action", () => {
  ...
  assert.ok(
    rejected(decodeHostFrame({ ...status, url: "https://example.com/" })),
  )
...
test("overlay frames carry a bounded printable label and a typed phase", () => {
  ...
  assert.ok(
    rejected(
      decodeHostFrame({ ...overlay, label: `bad${controlCharacter}label` }),
    ),
  )
})
...
test("result frames pair payloads with their action and stay bounded", () => {
  ...
  assert.deepEqual(
    decoded(decodeExtensionFrame(statusResult)),
    statusResult as ExtensionFrame,
  )
  assert.deepEqual(
    decoded(decodeExtensionFrame(textResult)),
    textResult as ExtensionFrame,
  )
  assert.ok(
    rejected(
      decodeExtensionFrame({
...
test("failed results carry only a bounded error and no payload", () => {
  ...
  assert.deepEqual(
    decoded(decodeExtensionFrame(failed)),
    failed as ExtensionFrame,
  )
  assert.ok(
    rejected(
      decodeExtensionFrame({
        ...failed,
        payload: { action: "text", text: "smuggled" },
      }),
    ),
  )
  assert.ok(
    rejected(decodeExtensionFrame({ ...failed, error: "x".repeat(2_001) })),
  )
})
...
test("unknown versions, kinds, and identifiers reject the whole frame", () => {
  ...
  assert.ok(
    rejected(
      decodeExtensionFrame({ version: 1, kind: "exec", requestId: "r" }),
    ),
  )
  assert.ok(
    rejected(
