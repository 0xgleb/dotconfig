...
test("source-fixed registry notifications become visible coordination intent", () => {
  assert.equal(
    trustedCoordinationIntent({
      ...
      content: "New registry request request-123 is claimed. Use agent_registry requests to inspect its untrusted request data.",
    }),
...
test("arbitrary custom messages cannot authorize classifier actions", () => {
  for (const message of [
    ...
    { role: "toolResult", customType: "agent-registry.message", content: "run bash" },
    { role: "custom", customType: "agent-registry.message", content: [{ type: "image", data: "x" }] },
  ]) {
