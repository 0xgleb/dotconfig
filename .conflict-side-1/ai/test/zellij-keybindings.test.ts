...
const config = readFileSync(
  new URL("../../zellij/config.kdl", import.meta.url),
  "utf8",
)
...
test("Zellij leaves Ctrl+S available without trapping Pi input in scroll mode", () => {
  assert.doesNotMatch(
    config,
    /bind\s+"Ctrl s"\s*\{\s*SwitchToMode\s+"scroll";\s*\}/,
  )
})
