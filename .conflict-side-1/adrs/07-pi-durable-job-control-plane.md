...
For subscription usage, the operator questions are deliberately bounded:

- Is fleet sampling healthy, and what cumulative usage survives agent expiry?
- Is observed weekly allowance burn ahead of or behind the straight-line runway
  to the verified reset timestamp?
- Which bounded project grouping accounts for the current allocation?

Each question has one signal: sampling status plus durable counter snapshots, a
verified allowance-checkpoint trajectory, and the dashboard's project
attribution respectively. Raw prompts, model output, and credentials never enter
these signals.
