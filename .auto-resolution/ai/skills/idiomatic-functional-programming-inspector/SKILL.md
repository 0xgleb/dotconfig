...
description: Auto-run alongside applicable language inspectors when reviews or audits touch source code with transforms, state, effects, or domain models; flag hidden effects, shared mutation, exception control flow, partial functions, and invalid-state models, but skip docs/config-only changes.
