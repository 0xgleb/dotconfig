---
name: graphite
description: When a repository is Graphite-managed or the user invokes `gt`, use Graphite in both main and linked worktrees to create, navigate, restack, and submit stacked PR branches; do not use it for GitButler repositories or ordinary worktree creation.
allowed-tools:
  - "Bash(gt *)"
  - "Bash(git add *)"
  - "Bash(git reset *)"
...
requests. Graphite remains the VCS backend in linked worktrees; unlike
GitButler, it is not restricted to the main worktree.
