...
description: Review code that represents, accounts for, or moves money for financial-programming defects — flags floating point, wrong rounding, unit and scale confusion, precision loss, overflow, non-idempotent movement, broken accounting identities, and cross-ledger reconciliation gaps. Auto-runs when a review or audit touches amounts, balances, prices, fees, ledger entries, equity, NAV, cash flows, or monetary projections.
...
7. **Accounting identities must close.** Deposits, withdrawals, fees, funding,
   realized returns, unrealized returns, liabilities, and current equity must
   form one explicit identity at one valuation basis. Monetary subledgers and
   projections need cross-ledger reconciliation, not independent formulas that
...
- **Accounting and projections:** deposits/withdrawals, realized/unrealized
  returns, fees/funding, liabilities, equity, NAV, protocol exposure, position
  totals, and every conversion or residual used to reconcile them.
...
### Accounting identities and cross-ledger reconciliation

- Deposits minus withdrawals plus returns minus fees/funding/liabilities does
  not provably reconcile to current equity on the same units and valuation
  basis.
- Monetary subledgers, position sums, exposure totals, and NAV use inconsistent
  signs, scales, marks, residual treatment, or inclusion rules.
- One monetary projection is recomputed independently instead of deriving from
  or reconciling against the canonical ledger/book.
- Reconciliation tolerances are unspecified, wider than the smallest-unit and
  valuation policy justify, or silently absorb real value drift.

The defensive-programming inspector owns the broader cross-view/system-state
proof. This inspector owns the monetary identity, units, rounding, signs, and
residual policy that make that proof financially correct.
...
1. <file>:<line>  [severity]
   Category: <representation | rounding | units-scaling | overflow-cast | movement-invariant | accounting-identity | cross-ledger-reconciliation>
...
1. **Stay in the money lane.** Representation, arithmetic, scaling,
   conservation, accounting identities, cross-ledger reconciliation, and
   idempotency. No trading execution logic, statistical strategy validation,
   risk limits, or general correctness.
