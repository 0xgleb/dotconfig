...
description: Review code paths that can move money or accumulate exposure for missing risk controls — flags absent position and exposure limits, no kill switch or circuit breaker, fail-open error handling, retry loops that can duplicate orders or payments, unvalidated external prices driving decisions, unbounded order rates, and limit breaches that are silently clamped instead of surfaced. Auto-runs when a review or audit touches order placement, payment dispatch, position sizing, or automated money-moving loops.
...
1. <file>:<line>  [severity]
   Category: <missing-bounds | fail-open | duplicate-side-effects | unvalidated-input | silent-breach>
...
1. **Stay in the risk-controls lane.** Bounds, failure posture, stops,
   idempotency of side effects, breach visibility. Not logic correctness,
   not representation.
