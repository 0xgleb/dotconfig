...
const componentStyles = (
  config: string,
): ReadonlyMap<string, { base: string; background: string }> => {
...
test("the selected theme is defined inline so running sessions pick it up", () => {
  ...
  assert.match(
    config,
    new RegExp(`themes\\s*\\{[\\s\\S]*?\\b${name}\\s*\\{`),
    `${name} must be defined in config.kdl`,
  )
  assert.ok(
...
test("the bar sits on exactly the terminal background, with no step at the edge", () => {
  ...
  const terminal = explicit
    ? (explicit[1] as string)
    : ITERM2_DEFAULT_BACKGROUND

  if (!explicit)
    assert.match(
      ghostty,
      /^theme\s*=\s*iTerm2 Default$/m,
      "terminal background assumption changed",
    )
...
test("every highlight is actually visible against the bar", () => {
  ...
  for (const component of HIGHLIGHTS) {
    ...
    assert.ok(
      luminance(fill) > luminance(bar),
      `${component} must sit above the bar, not below it`,
    )
    assert.ok(
...
test("the panels form a depth ramp rather than one flat repaint", () => {
  ...
  assert.ok(
    bar < unselected && unselected < selected,
    "expected bar < unselected panel < selected panel",
  )
})
...
test("no broad chrome surface is a bright panel", () => {
  const bright = [...componentStyles(read(CONFIG))]
    ...
    .filter(
      ([, { background }]) => luminance(background) > MAX_SURFACE_LUMINANCE,
    )
  ...
  assert.deepEqual(
    [...new Set(bright)],
    [],
    "a broad fill this light is the glare the user rejected",
  )
})
...
const archeofuturismPalette = (): ReadonlySet<string> => {
  ...
  const palette = new Set(
    [
      ...Object.values(piTheme.vars),
      ...Object.values(piTheme.colors),
      ...Object.values(piTheme.export),
    ]
...
test("the terminal palette draws from the archeofuturist palette", () => {
  ...
  const declared = [
    ...ghostty.matchAll(
      /^(?:palette\s*=\s*\d+|foreground|cursor-color|selection-\w+)\s*=\s*(#[0-9A-Fa-f]{6})/gm,
    ),
  ].map(([, color]) => (color as string).toUpperCase())

  assert.ok(
    declared.length >= 16,
    "expected the full 16-slot ANSI palette to be declared",
  )
  assert.deepEqual(
...
test("zellij chrome draws from the same palette as the Pi theme", () => {
  ...
  const foreign = [
    ...(themeBlock[0] as string).matchAll(/"(#[0-9A-Fa-f]{6})"/g),
  ]
    .map(([, color]) => (color as string).toUpperCase())
...
test("zellij keeps the arrow separators and the rounded pane-frame treatment", () => {
  ...
  assert.match(
    config,
    /ui \{[\s\S]*?pane_frames \{[\s\S]*?rounded_corners true/,
  )
  assert.doesNotMatch(
    config,
    /^simplified_ui true/m,
    "simplified_ui strips the arrow separators from the mode panels",
  )
})
