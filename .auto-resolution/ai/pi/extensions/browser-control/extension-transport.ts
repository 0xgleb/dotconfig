...
export const decideHandshake: (
  ...
  if (request.method !== "GET") return rejected("handshake must be a GET request");
  ...
  return {
    outcome: "accepted",
    acceptKey: createHash("sha1").update(key + WEBSOCKET_GUID).digest("base64"),
  };
};
...
export const decodeClientFrames: (
  ...
  while (buffer.length - offset >= 2) {
    ...
    if ((first & 0x80) === 0) return violation("fragmented frames are not supported");
    ...
    if (payloadLength === 126) {
      ...
      payloadLength = ((buffer[offset + 2] ?? 0) << 8) | (buffer[offset + 3] ?? 0);
      headerLength = 4;
    } else if (payloadLength === 127) {
      return violation("sixty-four bit frame lengths exceed the transport bound");
    }
    if (payloadLength > maxPayloadBytes) return violation("frame exceeds the payload bound");
    ...
    for (let index = 0; index < payloadLength; index += 1) {
      payload[index] =
        (buffer[offset + headerLength + 4 + index] ?? 0) ^ (mask[index % 4] ?? 0);
    }
    if (opcode === 0x1) {
      let text: string;
      try {
...
export const encodeTextFrame: (text: string) => Uint8Array = (text) => {
  ...
  if (payload.length > 0xffff) {
    throw new Error("server frames are bounded to sixteen-bit lengths");
  }
...
export const encodePongFrame: (payload: Uint8Array) => Uint8Array = (payload) => {
  if (payload.length >= 126) throw new Error("control frame payloads are bounded to 125 bytes");
  const frame = new Uint8Array(2 + payload.length);
  frame[0] = 0x8a;
  frame[1] = payload.length;
  frame.set(payload, 2);
...
export const encodeCloseFrame: () => Uint8Array = () => Uint8Array.from([0x88, 0x00]);

export const mintTransportToken: (entropy: Uint8Array) => string = (entropy) => {
  if (entropy.length < TRANSPORT_TOKEN_ENTROPY_BYTES) {
    throw new Error(
...
export const decideHello: (
  ...
  if (Either.isLeft(frame)) return { outcome: "rejected", reason: "hello frame is malformed" };
  ...
  if (offered.length !== expected.length || !timingSafeEqual(offered, expected)) {
    return { outcome: "rejected", reason: "token does not match the current provisioning" };
  }
  if (frame.right.profilePath !== provisioned.profilePath) {
    return { outcome: "rejected", reason: "profile is not the operator profile" };
  }
...
const rejected = (reason: string): HandshakeDecision => ({ outcome: "rejected", reason });

const violation = (reason: string): ClientFrameResult => ({ outcome: "violation", reason });

const isSixteenByteBase64 = (value: string): boolean => {
  try {
    return Buffer.from(value, "base64").length === 16 && btoa(atob(value)) === value;
  } catch {
    return false;
  }
};
