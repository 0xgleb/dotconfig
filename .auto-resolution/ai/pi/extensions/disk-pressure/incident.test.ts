...
test("only one session claims a fresh resource-pressure incident", () => {
  ...
  try {
    assert.equal(Effect.runSync(claimResourceIncident(path, "session-a", 1_000, 60_000)), true);
    assert.equal(Effect.runSync(claimResourceIncident(path, "session-b", 1_001, 60_000)), false);
...
test("stale incidents can be reclaimed and healthy state clears the lease", () => {
  ...
  try {
    assert.equal(Effect.runSync(claimResourceIncident(path, "session-a", 1_000, 60_000)), true);
    assert.equal(Effect.runSync(claimResourceIncident(path, "session-b", 61_001, 60_000)), true);
    Effect.runSync(clearResourceIncident(path));
    assert.equal(Effect.runSync(claimResourceIncident(path, "session-c", 61_002, 60_000)), true);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
