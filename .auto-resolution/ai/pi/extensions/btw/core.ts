...
const clip = (text: string, limit: number): string =>
  text.length <= limit ? text : `${text.slice(0, Math.max(0, limit - 16))}\n…[truncated]`;

const transcriptSection = (entry: TranscriptEntryLike, toolResultLimit: number): string | undefined => {
...
export const buildBoundedTranscript = (
  ...
  for (let index = sections.length - 1; index >= 0 && remaining > 0; index -= 1) {
    ...
    kept.unshift(remaining <= marker.length
...
export const sideQuestionPrompt = (question: string, transcript: string): string => [
