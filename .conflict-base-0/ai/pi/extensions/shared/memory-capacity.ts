...
export const availableMemoryBytes = (dependencies: MemoryCapacityDependencies = {}): bigint => {
  if ((dependencies.platform ?? process.platform) !== "darwin") {
    return BigInt((dependencies.freeMemoryBytes ?? freemem)());
  }
  const result = (dependencies.memoryPressureProbe ?? defaultMemoryPressureProbe)();
  if (result.status !== 0) throw new Error("macOS available-memory probe failed");
