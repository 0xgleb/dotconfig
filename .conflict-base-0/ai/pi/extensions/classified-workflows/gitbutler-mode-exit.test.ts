...
const diagnosticBranch = (diagnostic: string, command = "but status -f --format agent") => [
  {
    type: "message",
    message: {
      role: "assistant",
...
const required = "Error: GitButler mode exit required: please run but teardown to preserve your work.";
...
test("exact current GitButler mode-exit diagnostics override a necessity-only deadlock", () => {
  assert.equal(
    requiredGitButlerModeExitDisprovesBlock({
      reason: "Teardown is not necessary; use the initialized GitButler workflow instead.",
...
test("a newer contradictory GitButler diagnostic keeps teardown blocked", () => {
  const branch = [
    ...diagnosticBranch(required),
    {
      ...
      message: {
        role: "assistant",
        content: [{ type: "toolCall", id: "call-2", name: "bash", arguments: { command: "but status -f --format agent" } }],
      },
    },
    {
      type: "message",
      message: {
        ...
        toolCallId: "call-2",
        isError: false,
        content: [{ type: "text", text: "GitButler workspace is initialized." }],
      },
    },
  ];
  assert.equal(
