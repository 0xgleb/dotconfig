...
const editInput = (value: unknown): EditInput | undefined => {
  if (!isRecord(value) || typeof value.path !== "string" || !Array.isArray(value.edits)) return undefined;
  const edits = value.edits.flatMap((entry) =>
    isRecord(entry) && typeof entry.oldText === "string" && typeof entry.newText === "string"
      ? [{ oldText: entry.oldText, newText: entry.newText }]
      : [],
  );
  return edits.length === value.edits.length && edits.length > 0 ? { path: value.path, edits } : undefined;
};
  ...
  if (prior.name === "edit" && priorEdit) {
    return proposed.edits.every((replacement) =>
      priorEdit.edits.some(
        (earlier) => replacement.oldText === earlier.newText && replacement.newText === earlier.oldText,
      ));
  }
  if (prior.name !== "write" || !isRecord(prior.input) || typeof prior.input.content !== "string") return false;
  return proposed.edits.length === 1 &&
