...
test("human steering detaches a foreground workflow", () => {
  assert.equal(shouldDetachForegroundWorkflow("steer", "interactive", true), true)
...
test("follow-ups and extension messages do not detach foreground work", () => {
  ...
  assert.equal(shouldDetachForegroundWorkflow("steer", "extension", true), false)
  assert.equal(shouldDetachForegroundWorkflow("steer", "interactive", false), false)
})
